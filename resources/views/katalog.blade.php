    ...
    <nav class="bg-white/90 backdrop-blur shadow-sm px-6 py-3 flex justify-between items-center border-b border-indigo-100 sticky top-0 z-50">
        ...
        <div class="flex items-center gap-4">
            ...
            <form action="{{ route('logout') }}" method="POST" class="inline" id="logout-form">
                @csrf
                <button type="button" onclick="konfirmasiLogout()" class="text-sm text-red-500 font-semibold hover:text-red-700 transition px-3 py-1.5 rounded-lg hover:bg-red-50">
                    Logout
                </button>
            </form>
        </div>
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        @else
                            <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-5" id="book-grid">
                                ...
                                <div class="book-card bg-white border border-gray-100 rounded-2xl flex flex-col overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                                    
                                    <a href="/books/{{ $book->id }}" class="block group cursor-pointer flex-1 flex flex-col">
                                        
                                        <div class="h-48 bg-gray-200 flex items-center justify-center group-hover:bg-gray-200 transition-colors">
